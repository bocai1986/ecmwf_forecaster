...
def extract_cycle_from_filename(path: str) -> datetime:
    """
    从文件名里抽 init_tag（YYYYMMDDHH），比如：
      ec_upper_2025111300_3h_0-72_0p25.grib2
      ec_surface_2025111300_6h_0-240_0p25.grib2
    抽到 2025111300 → 返回带 UTC 的 datetime
    """
    import re
    basename = os.path.basename(path)
    m = re.search(r"(\d{10})", basename)
    if not m:
        raise ValueError(f"无法从文件名中解析起报时间：{basename}")
    tag = m.group(1)
...
def plot_t_gh_uv_one_time(fields, level_hpa, step_hour,
    ...
    # =====  上空风场改为箭头（quiver），风速 ≥12 m/s 红色  =====
    # stride = barb_stride if barb_stride is not None else 8
    # lon_q = lon2d[::stride, ::stride]
    # lat_q = lat2d[::stride, ::stride]
    # u_q = uu[::stride, ::stride]
    # v_q = vv[::stride, ::stride]
    ...
    #     # ≥12 m/s 红色，其余黑色
    #     colors = np.where(speed >= 12.0, "red", WIND_BLACK)
...
#     """
#     批量绘制地面产品：
#     1) 降水相态 + 数值（plot_surface_one_time）
#     2) MSLP + 10m 风箭头（plot_surface_synoptic）

#     目录结构：
#       output_root/init_tag/step6h/surface/phase/
#       output_root/init_tag/step6h/surface/synoptic/
#     """
...
#     if anchor is None:
#         raise ValueError(
#             "plot_surface_from_file(): surface_fields 缺少 msl/sp/tp/t2m/2t 任一可用锚点。"
#         )
...
#     else:
#         desired = np.arange(
#             0, int(max_hour) + 1, int(step_interval_hours), dtype=int
#         )
...
#         output_root, init_tag, f"step{int(step_interval_hours)}h", "surface"
#     )
#     dir_phase = os.path.join(base_dir, "phase")     # 降水相态图
#     dir_synop = os.path.join(base_dir, "synoptic")  # MSLP + 风场图
#     os.makedirs(dir_phase, exist_ok=True)
#     os.makedirs(dir_synop, exist_ok=True)

#     print(f"📁 输出目录（phase）:   {dir_phase}")
#     print(f"📁 输出目录（synoptic）:{dir_synop}")
...
#         out_png = os.path.join(
#             dir_synop,
#             f"EC_SurfaceSynop_{init_tag}_plus{h:03d}h.png"
#         )
...
#     print(
#         f"✅ 地面批量绘图完成：{img_count} 张 | "
#         f"总耗时 {total_sec:.2f}s | ≈ 每张 {per_img:.2f}s"
#     )
...
#     # 作为锚点的任意变量
#     anchor = _pick_first(surface_fields, ["msl", "tp", "2t", "10u", "10v"])
#     if anchor is None:
#         raise ValueError(
#             "plot_surface_from_file(): surface_fields 缺少 msl/sp/tp/t2m/2t 任一可用锚点。"
#         )
...
def plot_surface_from_file(
    ...
    *,
    ...
    steps_avail = steps_h.astype(int)
    steps_set = set(steps_avail.tolist())  # 目前没用到，但以后要查“有没有上一帧”可以用它

    print(f"📦 文件里实际有的 step(h): {steps_avail}")
    ...
    for h in steps_to_plot:
        ...
        plot_surface_one_time(
            ...
            interval_hours=int(step_interval_hours),
            accum_alignment=accum_alignment,
            label_stride=10,           # 这一行你可以继续按间隔动态调
        )
...
if __name__ == "__main__":
    ...
    OUT_ROOT = "/data/ecmwf/upper_airfields"  # 上空 + 地面绘图结果根目录

    # （可选）已有的 GRIB 文件路径，不想重下时手动填上；为空则继续自动下载
    EXISTING_UPPER_PATH   = ""
    ...
    for interval, maxh in JOBS:
        ...
        # ================== 1) 上层 GRIB：已有 → 用已有；否则下载 ==================
        if EXISTING_UPPER_PATH:
            upper_path = resolve_surface_path(EXISTING_UPPER_PATH)
            cycle_utc = extract_cycle_from_filename(upper_path)
            print(f"📁 使用已有 upper GRIB: {upper_path}")
        else:
            ...
            print(f"✅ 上层 GRIB 下载完成: {upper_path}")

        init_tag = cycle_utc.strftime("%Y%m%d%H")
        print(f"🕒 起报时间: {init_tag}")

        # ================== 2) 地面 GRIB：已有 → 用已有；否则锚定起报下载 ==================
        if EXISTING_SURFACE_PATH:
            # surface_path = resolve_grib_path_auto(EXISTING_SURFACE_PATH, want="surface")
            surface_path = resolve_surface_path(EXISTING_SURFACE_PATH)
            print(f"📁 使用已有 surface GRIB: {surface_path}")
        else:
            surface_path, _, _ = retrieve_combined(
                client, SAVE_DIR,
                step_interval_hours=interval,
                max_hour=maxh,
                params=["msl", "tp", "2t", "10u", "10v"],
                level_list=None,
                init_from_latest=False,
                anchor_cycle_utc=cycle_utc,
            )
            print(f"✅ 地面 GRIB 下载完成: {surface_path}")

        print(f"🔗 上下同一轮起报: {init_tag}")
