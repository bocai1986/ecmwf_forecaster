...
用法：
  ...
  # 不传 root 时，自动挑选最近就绪的根目录（/data/ecmwf/upper_airfields/YYYY-MM-DD/YYYYMMDD12）
"""
...
def video_props(path: Path):
    ...
    return dict(fps=round(fps, 3), width=w, height=h, frames=n,
...
def file_stats(path: Path):
    ...
    return dict(
        bytes=st.st_size,
        mtime_iso=datetime.fromtimestamp(st.st_mtime, ZoneInfo("Asia/Shanghai")).isoformat()
    )
...
def scan_group(root: Path, group_name: str, cfg: dict, base_url: str | None):
    ...
    for layer in LAYER_ORDER:
        layer_name = f"{layer}_{cfg['tag']}.mp4"
        p = layers_dir / layer_name
        if not p.exists():
            # 兼容大小写或连字符差异（保险处理）
            cand = list(layers_dir.glob(f"*{layer.split('hPa')[0]}*{cfg['tag']}*.mp4")) + \
                   list(layers_dir.glob(f"*{layer.lower()}*{cfg['tag']}*.mp4"))
            if cand:
                p = cand[0]
        if p.exists():
            props = video_props(p)
            ...
            print(f"ℹ️ 缺少层视频：{layers_dir}/{layer_name}")
...
def main():
    ...
    ap.add_argument("root", nargs="?", help="根目录（包含 products 的那一层）；不传则自动挑选最近就绪")
    ap.add_argument("--out", default=None, help="manifest 输出路径（默认：<root>/products/manifest.json）")
    ap.add_argument("--base-url", default=None, help="可选：为 rel_path 补前缀生成可访问 URL（如 https://cdn.example.com/ecmwf）")
    ap.add_argument("--data-root", default=DATA_ROOT_DEFAULT, help=f"自动挑选模式时的起点（默认：{DATA_ROOT_DEFAULT}）")
    args = ap.parse_args()
    ...
    for g in manifest["groups"]:
        ...
        print(f" • {g['name']}（step={g['step']} tag={g['tag']}） combined={c}  layers={list(g['layers'].keys())}")
